...
| 1.14.0 | 2026-09-20 JST | 架构师 (Mavis 接手 agent per DEC-008) | D-1 + C-6 双双实装落地 + merge main (clean fast-forward from `wt/lane-config-wireup` HEAD `138b043`). 新增 §v1.14.0 增量: D-1 `AppConfig::load()` + main.rs wire-up (commit `83ec871`) + C-6 `link_account` UPDATE SQL (commit `8ca9b24`, Ulysses 直签) + v1.14.0 docs (commit `138b043`). 测试 cargo check -p im-core 0 errors. 关键路径剩余 ~0.1-0.6M tokens. cleanup `wt/lane-config-wireup` + `wt-ulys-122` worktree/分支. 修订者: Mavis 父代理亲自推 (merge to main + cleanup). |

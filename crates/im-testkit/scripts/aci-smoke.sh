#!/usr/bin/env bash
# IM1.0 aci-smoke.sh v0.1 — ULYS-191 §4.3.3 brief v0.1
#
# 4 step verify (per RGS §4.3.1 + IDE1.0 cli_smoke.sh 1:1 pattern):
#   1. cargo build -p im-testkit  (含 aci-emitter git dep)
#   2. cargo test -p im-testkit --test aci_integration test_im_testkit_emits_valid_aci_assertion -- --nocapture
#      (IM1.0 是 library, 没有 CLI, smoke 走 cargo test 间接 emit)
...
echo "=== IM1.0 aci-smoke.sh v0.1 ==="
echo "WORKDIR=$WORKDIR"
echo

# Step 1: 编译 (含 aci-emitter git dep)
echo "[step 1/4] cargo build -p im-testkit"
cargo build -p im-testkit -j 2 2>&1 | tail -5
echo

# Step 2: emit (通过 cargo test --nocapture 触发 emit_smoke_assertion_json)
echo "[step 2/4] cargo test -p im-testkit --test aci_integration test_im_testkit_emits_valid_aci_assertion -- --nocapture"
# 抓 IT-2 的 stdout 输出 (=== emit_smoke_assertion JSON === ... === END ===)
JSON_OUT="$(cargo test -p im-testkit --test aci_integration test_im_testkit_emits_valid_aci_assertion -- --nocapture 2>&1 || true)"
echo "$JSON_OUT" | grep -E "(test result|test_im_testkit_emits_valid_aci_assertion|emit_smoke_assertion JSON)" | head -10
echo

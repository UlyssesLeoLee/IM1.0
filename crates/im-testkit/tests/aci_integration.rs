//! `aci_integration` — 3 IT (per ULYS-191 §4.3.3 brief v0.1)
//!
//! 与 IDE1.0 §4.2.2 + RGS §4.3.1 pattern 1:1 (per brief v0.1 §3.2 #19):
//!
//! | IT  | 名称                                       | 校验                                                                                 |
//! |-----|--------------------------------------------|--------------------------------------------------------------------------------------|
//! | IT-1 | `test_aci_schema_v0_1_roundtrip`            | `.aci.json` schema v0.1 与 aci-emitter 完全一致 (字段名 1:1)                         |
//! | IT-2 | `test_im_testkit_emits_valid_aci_assertion` | `emit_smoke_assertion()` + `emit_ws_frame_assertion(...)` 输出 assertion 含全部 10 必填字段 + 字段名 1:1 对齐 |
//! | IT-3 | `test_aci_emitter_v0_1_compatibility`       | IM1.0 Rust emitter 字段名与 Star Python emitter 跨语言 parity (字段名 1:1)          |
//!
//! 守门 (per AGENTS.md §4):
//! - #13 W/T/M: 单元 (aci_emitter_helper 3 单测) + 集成 (本文件 3 IT) + 系统 (aci-smoke.sh)
...
fn test_aci_schema_v0_1_roundtrip() {
    ...
    // 3) project 字段 == im1.0 (与 scope.project 一致)
    let project = schema["project"]
        .as_str()
        .expect("schema.project must be a string");
    assert_eq!(
        project, "im1.0",
        "schema.project must be 'im1.0' (per scope.project)"
    );

    // 4) 17 个 expect_value_types
    ...
    assert_eq!(
        evts.len(),
        17,
        "schema.expect_value_types must have 17 entries (per §4.3 v0.1)"
    );

    // 5) 6 个 scope_dimensions
    ...
    // 6) 10 个 schema_required_fields — 字段名 1:1 对齐
...
/// IT-2: `emit_smoke_assertion()` + `emit_ws_frame_assertion(...)` 输出 assertion 含全部 10 必填字段 + 字段名 1:1 对齐.
///
/// 校验项:
/// - 10 必填字段全部存在且字段名与 schema_required_fields 1:1 对齐
/// - 默认值符合 §4.3.3 brief v0.1 (assertion_id="im1.0:smoke:g-1" 等)
/// - JSON 序列化字段名 1:1 (与 Python emitter 跨语言可解析)
/// - emit_ws_frame_assertion 配对 im-testkit `assert_ws_frame_shape` (字段名 parity)
#[test]
fn test_im_testkit_emits_valid_aci_assertion() {
    let assertion = aci_emitter_helper::emit_smoke_assertion();

    // 1) 10 必填字段非空
    assert_eq!(assertion.assertion_id, "im1.0:smoke:g-1");
    assert_eq!(assertion.aci_version, "0.1.0-draft");
    assert_eq!(assertion.layer, Layer::It);
    assert_eq!(assertion.scope.project, "im1.0");
    ...
    assert!(
        ...
        "reasoning must be non-empty (per §4.3.3)"
    );
    assert!(
    ...
    // 2) expect/actual 字段类型 + 值 (response_within_ms 100/15, per §4.3.3 brief)
    assert_eq!(
        assertion.expect.value_type,
        ExpectValueType::ResponseWithinMs
    );
    assert_eq!(assertion.expect.value, serde_json::json!(100));
    assert_eq!(
        assertion.actual.value_type,
        ExpectValueType::ResponseWithinMs
    );
    assert_eq!(assertion.actual.value, serde_json::json!(15));
    ...
    // 4) emit_ws_frame_assertion 字段名与 assertions::assert_ws_frame_shape 配对
    let ws_assertion = aci_emitter_helper::emit_ws_frame_assertion("connected");
    assert_eq!(ws_assertion.assertion_id, "im1.0:ws:connected");
    assert_eq!(ws_assertion.scope.project, "im1.0");
    assert_eq!(ws_assertion.scope.module.as_deref(), Some("ws"));
    assert_eq!(ws_assertion.expect.value_type, ExpectValueType::FieldEquals);
    assert_eq!(ws_assertion.expect.value, serde_json::json!("connected"));

    // 5) smoke helper 单独打印 JSON (供 aci-smoke.sh 抓 stdout)
    let pretty = aci_emitter_helper::emit_smoke_assertion_json();
    assert!(pretty.contains("im1.0:smoke:g-1"));
    eprintln!("=== emit_smoke_assertion JSON ===\n{pretty}\n=== END ===");
}

/// IT-3: IM1.0 Rust emitter 与 Star Python emitter 跨语言 parity (字段名 1:1)。
///
/// 校验项 (per §4.3 Python emitter + brief v0.1):
...
fn locate_aci_json() -> PathBuf {
    // CARGO_MANIFEST_DIR points to crates/im-testkit during cargo test

...
impl UserRepository for PgUserRepository {
    ...
    /// C-4: 按 (env, username) 查 user — 用于 authenticate
    ...
    /// C-3: 创建带 username + password_hash 的 user
    /// dup username → PG 23505 unique violation → map_sqlx_error → AccountAlreadyExists
